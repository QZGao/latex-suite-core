  ...
  highlightRange(cursor, from, to, inheritedClass, highlighters) {
    ...
    if (mounted && mounted.overlay) {
      ...
      for (let i = 0, pos = start2; ; i++) {
        ...
        pos = next.to + start2;
        if (pos > from) {
          this.highlightRange(inner.cursor(), Math.max(from, next.from + start2), Math.min(to, pos), inheritedClass, innerHighlighters);
...
var StringQuery = class extends QueryType2 {
  ...
  prevMatchInRange(doc2, from, to) {
    for (let pos = to; ; ) {
      ...
      let cursor = stringCursor(this.spec, doc2, start2, pos), range = null;
...
var RegExpQuery = class extends QueryType2 {
  ...
  prevMatchInRange(doc2, from, to) {
    for (let size = 1; ; size++) {
      ...
      let cursor = regexpCursor(this.spec, doc2, start2, to), range = null;
      while (!cursor.next().done)
        range = cursor.value;
...
var startSnippet = StateEffect.define();
var endSnippet = StateEffect.define();
var undidStartSnippet = StateEffect.define();
var undidEndSnippet = StateEffect.define();
...
function replaceRange2(view, start2, end, replacement) {
  view.dispatch({
    changes: { from: start2, to: end, insert: replacement }
  });
}
function getCharacterAtPos(viewOrState, pos) {
  ...
  return doc2.slice(pos, pos + 1).toString();
}
...
function findMatchingBracket(text, start2, openBracket, closeBracket, searchBackwards, end) {
  if (searchBackwards) {
...
var SnippetChangeSpec = class {
  constructor(from, to, insert2, keyPressed) {
    this.from = from;
    this.to = to;
    this.insert = insert2;
  ...
  getTabstops(view, start2) {
    ...
    for (let i = start2; i < start2 + this.insert.length; i++) {
      ...
      let number3 = parseInt(text.charAt(i + 1));
      ...
      if (isNaN(number3)) {
        ...
        number3 = parseInt(tabstopString.slice(2, colonIndex));
        if (isNaN(number3))
          continue;
        if (closingIndex === -1)
          continue;
      ...
      const tabstop = { number: number3, from: tabstopStart, to: tabstopEnd, replacement: tabstopReplacement };
...
var queueSnippetEffect = StateEffect.define();
var clearSnippetQueueEffect = StateEffect.define();
var snippetQueueStateField = StateField.define({
  create() {
    return [];
  },
...
function queueSnippet(view, from, to, insert2, keyPressed) {
  const snippet = new SnippetChangeSpec(from, to, insert2, keyPressed);
  view.dispatch({
...
// _yo84x1axq:D:\OneDrive\Projects\2025_latex_suite\latex-suite-core\src\default_snippets.js
...
// _yo84x1axq:D:\OneDrive\Projects\2025_latex_suite\latex-suite-core\src\default_snippet_variables.js
...
function sortSnippets(snippets) {
  return snippets.map((snippet, i) => [getPriority(snippet), getTriggerLength(snippet), i]).sort(schwartzianSnippetCompare).map(([p, t2, i]) => snippets[i]);
}
...
async function parseSnippetVariables(snippetVariablesStr) {
...
async function parseSnippets(snippetsStr, snippetVariables) {
...
var RawSnippetSchema = object({
  trigger: union([string2(), instance(RegExp)]),
  replacement: union([string2(), special((x) => typeof x === "function")]),
  options: string2(),
  flags: optional(string2()),
  priority: optional(number2()),
...
function parseSnippet(raw, snippetVariables) {
  ...
  if (options.regex || raw.trigger instanceof RegExp) {
    let triggerStr;
    let flags = (_a2 = raw.flags) != null ? _a2 : "";
...
var TabstopGroup = class {
  ...
  toSelectionRanges() {
    const ranges = [];
    const cur2 = this.decos.iter();
    while (cur2.value != null) {
      ranges.push(EditorSelection.range(cur2.from, cur2.to));
      cur2.next();
    }
    return ranges;
  }
  ...
  getRanges() {
    ...
    const cur2 = this.decos.iter();
    while (cur2.value != null) {
      if (cur2.from != cur2.to) {
        ranges.push(cur2.value.range(cur2.from, cur2.to));
      }
      cur2.next();
    }
    return ranges;
...
function tabstopSpecsToTabstopGroups(tabstops, color) {
  ...
  for (const number3 of numbers) {
    const grp = new TabstopGroup(tabstopsByNumber[number3], color);
    result.push(grp);
  }
  return result;
}
...
function handleUndoKeypresses(view, snippets) {
  ...
  for (const snippet of snippets) {
    if (snippet.keyPressed && snippet.keyPressed.length === 1) {
      const prevChar2 = view.state.doc.sliceString(snippet.to - 1, snippet.to);
      const from = snippet.to === 0 ? 0 : snippet.to - 1;
      keyPresses.push({ from, to: snippet.to, insert: prevChar2 + snippet.keyPressed });
    }
  }
  view.dispatch({
    changes: keyPresses,
...
var Context = class _Context {
  ...
  isWithinEnvironment(pos, env) {
    ...
    if (!bounds)
      return;
    const { start: start2, end } = bounds;
    const text = this.state.sliceDoc(start2, end);
    pos -= start2;
...
var langIfWithinCodeblock = (state) => {
  ...
  const language2 = state.sliceDoc(codeblockBegin.from, codeblockBegin.to).replace(/`+/, "");
  return language2;
};
...
var isOnWordBoundary = (state, triggerPos, to, wordDelimiters) => {
  const prevChar2 = state.sliceDoc(triggerPos - 1, triggerPos);
  const nextChar2 = state.sliceDoc(to, to + 1);
  wordDelimiters = wordDelimiters.replace("\\n", "\n");
  return wordDelimiters.includes(prevChar2) && wordDelimiters.includes(nextChar2);
};
...
var runAutoFractionCursor = (view, ctx, range) => {
  ...
  if (numerator.at(0) === "(" && numerator.at(-1) === ")") {
    const closing2 = findMatchingBracket(numerator, 0, "(", ")", false);
    if (closing2 === numerator.length - 1) {
...
var tabout = (view, ctx) => {
  ...
  if (ctx.mode.inlineMath || ctx.mode.codeMath) {
    ...
    if (dollarLine.number === d.lines) {
      replaceRange2(view, dollarLine.to, dollarLine.to, "\n");
    }
    setCursor(view, dollarLine.to + 1);
    const line = d.lineAt(pos);
    replaceRange2(view, line.from, line.to, line.text.trim());
  }
  return true;
};
...
var handleKeydown = (key, shiftKey, ctrlKey, isIME, view) => {
  ...
  if (settings.autoDelete$ && key === "Backspace" && ctx.mode.inMath()) {
    ...
    if (charAtPos === "$" && charAtPrevPos === "$") {
      replaceRange2(view, ctx.pos - 1, ctx.pos + 1, "");
    ...
    syntaxTree(view.state).iterate({
      ...
      enter: (node) => {
        ...
        for (let i = 0; i < eqn.length; i++) {
          ...
          } else if (closeBrackets2.includes(char)) {
...
function getEnclosingBracketsPos(view, pos) {
  ...
  if (!result)
    return -1;
  const { start: start2, end } = result;
  const text = view.state.doc.sliceString(start2, end);
...
async function start() {
  ...
  const cmSettings = processLatexSuiteSettings([...defaultSnips, ...demoExtraSnips], {
    ...DEFAULT_SETTINGS,
    concealEnabled: false,
    // core build excludes conceal implementation
    mathPreviewEnabled: false
    // core build excludes inline preview
  });
  ...
  const previewExtension = (el) => {
    let timer = null;
    const schedule = (view2) => {
      if (timer)
